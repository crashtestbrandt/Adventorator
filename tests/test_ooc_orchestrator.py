...
async def test_ooc_shadow_mode_ephemeral(monkeypatch):
    ...
    fake_out = LLMOutput(
        proposal=LLMProposal(
            action="ability_check", ability="DEX", suggested_dc=12, reason="""safe"""
        ),
        narration="narr",
    )
    ...
    body = {
        ...
        "token": "t",
        "application_id": "a",
        "data": {"name": "ooc", "options": [{"name": "message", "type": 3, "value": "hi"}]},
    }
    r = client.post(
        "/interactions",
    ...
    # Deferred response; follow-up happens out-of-band.
    # We can't capture follow-up here, but reaching here means no crash.
...
async def test_ooc_visible_path(monkeypatch):
    ...
    fake_out = LLMOutput(
        proposal=LLMProposal(action="ability_check", ability="STR", suggested_dc=10, reason="ok"),
        narration="text",
    )
    appmod.llm_client = FakeLLM(fake_out)
    ...
    body = {
        ...
        "token": "t",
        "application_id": "a",
        "data": {"name": "ooc", "options": [{"name": "message", "type": 3, "value": "go"}]},
    }
    r = client.post(
        "/interactions",
        content=json.dumps(body).encode(),
...
async def test_ooc_degraded_ephemeral(monkeypatch):
    ...
    body = {
        ...
        "token": "t",
        "application_id": "a",
        "data": {"name": "ooc", "options": [{"name": "message", "type": 3, "value": "uh"}]},
    }
    r = client.post(
        "/interactions",
        content=json.dumps(body).encode(),
