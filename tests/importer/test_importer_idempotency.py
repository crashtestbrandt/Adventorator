...
def _completion_payload(completion: object) -> dict:
    if hasattr(completion, "payload"):
        payload_holder = cast(Any, completion)
        return cast(dict, payload_holder.payload)
    if isinstance(completion, dict):
        return completion["payload"]
    raise AssertionError(f"Unexpected completion_event type: {type(completion)!r}")
...
class TestImporterIdempotency:
    ...
    def test_full_importer_idempotent_rerun(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            payload1 = _completion_payload(result1["completion_event"])
            payload2 = _completion_payload(result2["completion_event"])
    ...
    def test_metrics_instrumentation_idempotent_counter(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            get_counter("importer.idempotent")
