...
async def upsert_character(
    ...
    q = await s.execute(
        select(models.Character).where(
            models.Character.campaign_id == campaign_id, models.Character.name == sheet.name
        )
    )
    obj = q.scalar_one_or_none()
    if obj:
...
async def get_character(s: AsyncSession, campaign_id: int, name: str) -> models.Character | None:
    q = await s.execute(
        select(models.Character).where(
            models.Character.campaign_id == campaign_id, models.Character.name == name
        )
    )
...
async def get_recent_transcripts(
    ...
    """
    Fetch the most recent transcript entries for a given scene,
    optionally filtered by user_id, in chronological order.
    """
    ...
    results: Sequence[models.Transcript] = q.scalars().all()
    # Ensure concrete list for return type
