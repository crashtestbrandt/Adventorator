...
class SimpleExecutorPrototype:
    ...
    async def execute_with_idempotency(self, request: ExecutorRequest) -> ExecutorResult:
        ...
        existing_event = await self._find_existing_event(request.campaign_id, idempotency_key)
        ...
        try:
            ...
            # Create new event
            event = await self._create_event(request, idempotency_key, execution_result)
    ...
    async def _find_existing_event(self, campaign_id: int, idempotency_key: bytes):
        ...
        stmt = select(models.Event).where(
            models.Event.campaign_id == campaign_id, models.Event.idempotency_key == idempotency_key
        )
